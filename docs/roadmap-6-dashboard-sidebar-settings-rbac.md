...
### Phase 4 follow-ups (2026-07-12/13, reported by the user)

Three related fixes discovered by the user actually using a freshly created low-privilege account, in order:

- [x] **Team roster over-exposure**: `/api/team/permissions` and `/api/team/members` only required the broad `team:read` permission (granted to every role by default), so a viewer could see the entire staff roster (names, usernames, exact roles, departments, module assignments) and the full role x permission matrix. `/api/team/permissions` now requires an unscoped (supervisor/admin) caller. `/api/team/members` now conditionally projects: unscoped callers get the full record (needed for the Team management page), scoped callers get only `{id, name, isAvailable, department}` - enough for the assignment dropdowns on Conversations/Tickets/Flows that legitimately need this endpoint for every role. Fixed a resulting crash in the Team page's client-side search filter (`.toLowerCase()` on now-possibly-undefined fields).
- [x] **Sidebar showed every nav item to every role** regardless of actual access - confusing even though the underlying APIs correctly 403'd. `GET /api/auth` now also returns the caller's live permission set; each `NAV_SECTIONS` item carries an optional required permission; a new `filterSectionsByPermission()` hides gated items (and drops sections left empty), shared between the desktop sidebar and the mobile "More" sheet, re-fetched on every route change so a permission an admin just edited takes effect without reload.
- [x] **Header avatar/profile menu had no user awareness at all**: the avatar was a hardcoded "A" for every user, and "Profile & Settings" unconditionally linked to the admin-only instance Settings page regardless of role - a second, unguarded path to `/settings` the sidebar fix didn't cover. `Header` now fetches `/api/auth` too; avatar shows the real user's initial, the dropdown shows "Name / Role", and the settings shortcut (renamed "Instance Settings") only renders for callers with `settings:read`.

### Phase 4 policy change (2026-07-13, per explicit request): default permissions redesigned

Explicit instruction from the user: only admin should be able to configure admin-level settings; non-admin roles should default to conversations, tickets, and the reporter/chatbot only. Three decisions were confirmed before implementing (via AskUserQuestion, since this changes the security posture of every non-admin account):

1. **Keep the 4-role system** (viewer/agent/supervisor/admin) and the editable-permissions/custom-role infrastructure from earlier in this phase - this is a change to what the *built-in roles default to*, not an architectural rewrite back to 2 tiers.
2. **Full CRUD on conversations/tickets/reporter for working roles, nothing else** - this removes the previously-broad default access to Customers, Knowledge Base, Team, Automation, Agents, Webhooks, Settings, Analytics, Marketplace management, Channels, SLA, Business Hours, and Canned Responses from viewer/agent/supervisor. All of that becomes admin-only by default (still grantable per-role via the permission matrix built earlier in this phase).
3. Within that: `viewer` stays read-only on conversations/tickets/reporter (matching its name), `agent`/`supervisor` get full CRUD since they're the roles doing the actual support work, and `supervisor` additionally keeps `conversations:assign`/`conversations:delete`/`tickets:delete` (routing and closing out work) - it does not regain access to anything outside that scope.

Implementation: updated `DEFAULT_ROLE_PERMISSIONS` in `src/lib/rbac.ts` (the seed-data reference, not used for live authorization) and wrote migration `20260713064644_restrict_default_role_permissions` to delete and re-insert the `RolePermission` rows for viewer/agent/supervisor on the live DB to match (admin untouched).

**Bug caught by the QA pass, fixed same session**: the first version made `marketplace:read` admin-only along with `marketplace:install`/`marketplace:manage`. That silently broke Reporter access for every non-admin role - `GET /api/marketplace/modules` (which `marketplace:read` gates) is what the sidebar uses to list installed modules, including the Reporter Agent core module the reporter/chatbot lives in. A live sidebar check after the first migration showed the Reporter Agent link missing for a test viewer account, which is what caught it. Fixed by restoring `marketplace:read` to all roles via migration `20260713065413_restore_marketplace_read_for_reporter` - `marketplace:install`/`marketplace:manage` (actually installing/configuring modules) stay admin-only; the route itself already narrows the returned module list to core modules plus whatever the caller has an explicit `ModuleAssignment` for, so this doesn't reopen the module-visibility scoping, only the "can you ask the endpoint at all" gate. Accepted side effect: a non-admin can now browse the marketplace catalog (see what modules exist) even though they can't install/manage any - read-only, low-risk, and not worth further route-splitting to avoid.

**QA verification pass** (the user's explicit request - "introduce a QA engineer to check for everything"): created two disposable test team members (viewer, supervisor; the existing `e2e-agent` fixture covered agent) rather than reusing real seed members' credentials, after the harness correctly flagged that pattern as risky earlier in this session. Systematically checked all three non-admin roles against 15 previously-broad endpoints (customers, knowledge, team, automation, agents, webhooks, settings, analytics, marketplace, channels, sla, business-hours, canned-responses, admin, activity) - all 403 for all three roles, in both passes (before and after the marketplace:read fix). Confirmed conversations/tickets/reporter stay 200 for all three. Confirmed the read/write distinction live: viewer's `POST /api/conversations` 403s, agent's succeeds (201); viewer's `DELETE /api/tickets/[id]` 403s, supervisor's reaches the handler (404 for a nonexistent id, not 403). Confirmed admin is completely unaffected on every one of the same endpoints. Deleted both disposable test members afterward; live DB permission counts confirmed exactly correct (viewer 5, agent 12, supervisor 15, admin 68 unchanged) with zero leftover test data.
